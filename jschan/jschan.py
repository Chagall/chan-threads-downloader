...
def get_all_boards_data(chan_name, tld):
    ...
    for board in boards:
        ...
        for thread in board["threads"]:
            ...
            thread["replies"] = get_thread_replies(chan_name, tld, board_id, thread.get("id"))
...
def get_board_threads(chan_name, tld, board_id):
    """
    chan_name: 8chan, 27chan, 4chan, etc.
    tld: top level domain (.org, .com, etc)
...
def get_thread_replies(chan_name, tld, board_id, thread_id):
    """
    chan_name: 8chan, 27chan, 4chan, etc.
    tld: top level domain (.org, .com, etc)
    board_id: _id of the boards
    ...
    if response.status_code == 200:
        ...
        if thread.get("nomarkup") != None and thread.get("nomarkup") != "":
            ...
            if parsed_message != "":
                replies.append({"id": thread.get("postId"), "message": parsed_message})
            # Limpe o parser
            parser.clear()
        ...
        for reply in thread.get("replies"):
            if reply.get("nomarkup") != None and reply.get("nomarkup") != "":
                ...
                if parsed_message != "":
                    replies.append({"id": reply.get("postId"), "message": parsed_message})
            # Limpe o parser
            parser.clear()
